...
<div class="min-h-screen bg-gray-50">
	<div class="container mx-auto px-4 py-6 max-w-7xl">
		...
		<div class="text-center mb-6">
			<div class="inline-flex items-center justify-center w-16 h-16 bg-blue-600 rounded-2xl mb-4 shadow-sm">
			...
			<p class="text-gray-600 text-lg">Your AI-powered data analytics assistant</p>
		</div>
		...
		<div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
			...
			<div
				...
				class="h-[75vh] overflow-y-auto p-8 space-y-6 bg-white"
			>
				{#each messages as message (message.id)}
					<div class="flex {message.type === 'user' ? 'justify-end' : 'justify-start'}">
						<div class="max-w-2xl lg:max-w-4xl px-6 py-4 rounded-2xl {
							message.type === 'user' 
							...
							{#if message.type === 'sql'}
								<div class="flex items-center gap-2 text-sm font-semibold text-green-700 mb-3">
									<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
									</svg>
									SQL Query
								</div>
								<pre class="text-sm font-mono whitespace-pre-wrap overflow-x-auto bg-white p-4 rounded-lg border border-green-200">{message.content}</pre>
							{:else if message.type === 'data'}
								<div class="flex items-center gap-2 text-sm font-semibold text-blue-700 mb-4">
								...
								{#if message.data && message.data.length > 0}
									<div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
											...
											<table class="min-w-full text-sm">
												<thead class="sticky top-0 bg-gray-50">
													<tr class="border-b border-gray-200">
														{#each Object.keys(message.data[0]) as header}
															<th class="px-4 py-3 text-left font-semibold text-gray-700 border-r border-gray-200 last:border-r-0">
												...
												<tbody>
													{#each message.data.slice(0, 20) as row, index}
														<tr class="border-b border-gray-100 hover:bg-gray-50 {index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}">
															{#each Object.values(row) as cell}
																<td class="px-4 py-3 border-r border-gray-100 last:border-r-0">
																	{#if typeof cell === 'number'}
																		<span class="font-mono text-blue-600 font-semibold">{cell.toLocaleString()}</span>
																	{:else if cell === null}
										...
										{#if message.data.length > 20}
											<div class="text-sm text-amber-700 mt-4 p-3 bg-amber-50 rounded-lg border border-amber-200">
				...
				{#if isLoading}
					<div class="flex justify-start">
						<div class="bg-gray-100 text-gray-700 border border-gray-200 px-6 py-4 rounded-2xl shadow-sm">
				...
				<div class="flex space-x-4 items-end">
					<div class="flex-1">
						<textarea
							...
							class="w-full resize-none bg-white border border-gray-300 rounded-2xl px-6 py-4 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 shadow-sm"
							rows="3"
							disabled={isLoading}
						></textarea>
					</div>
					<button
						...
						class="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white px-8 py-4 rounded-2xl font-semibold transition-all duration-200 shadow-sm hover:shadow-md transform hover:scale-105 disabled:transform-none disabled:hover:scale-100 flex items-center gap-2"
					>
						<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
		...
		<div class="mt-8 bg-white rounded-2xl p-8 shadow-lg border border-gray-200">
			...
			<div class="grid md:grid-cols-2 gap-8">
				<div class="space-y-4">
					...
					<ul class="text-gray-600 space-y-2 ml-11">
